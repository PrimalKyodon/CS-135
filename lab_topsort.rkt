...
(define (indegree E v)
  (cond
    [(null? E) 0]
    [(eq? (cadar E) v) (+ 1 (indegree (cdr E) v))]
...
;; 5 PTS
(define (find-top G)
  (define (find-top-help V E)
    (cond
      [(null? V) (topsort-error)]
      [(eq? (indegree E (car V)) 0) (car V)]
      [else (find-top-help (cdr V) E)])
    )
  (find-top-help (get-vertices G) (get-edges G))
)
...
(define (pop-vertex G v)
  (list (popv-vertices (car G) v) (popv-edges (cadr G) v))
)
...
(define (topsort G)
  (cond
    [(null? (get-vertices G)) '()]
    [else (cons (find-top G) (topsort (pop-vertex G (find-top G))))])
)
