...
(define (intersection S1 S2)
  (cond
    [(null? S1) '()]
    [(null? S2) '()]
    [(element? S2 (car S1)) (append (list (car S1)) (intersection (cdr S1) S2))]
...
;; 3 PTS
(define (subset? S1 S2)
  (equal? (cardinality  S1) (cardinality (intersection S1 S2)))
)
...
(define (set-equal? S1 S2)
  (equal? (subset? S1 S2) (subset? S2 S1))
)
...
(define (reflexive? n R)
  (equal?  (cardinality (intersection (id n) R)) n)
)
