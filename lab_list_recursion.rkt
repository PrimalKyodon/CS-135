...
(define (rotate c)
  (define x (car c))
  (define y (car (cdr c)))
  (list c (list y (- x)) (list (- x) (- y)) (list (- y) x))
)
...
(define (combine-courses s1 s2)
  (define scl1 (cadr (cddddr s1)))
  (define scl2 (cadr (cddddr s2)))
  (append scl1 scl2)
)
...
;; 3 PTS
(define (sum-pow p n)
  (if (positive? n) (+ (expt n p) (sum-pow p (- n 1))) 0)
)
