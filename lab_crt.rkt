...
(define (CRT-exists? L)
  (cond
    [(null? (cdr L)) #t]
    [(= (gcd (find-m (cdr L)) (cadar L)) 1) (CRT-exists? (cdr L))]
    [else #f])
)
...
(define (CRT-helper L m)
  (define a (caar L))
  (define b (cadar L))
  (define i (* a (/ m b) (mod-inverse (/ m b) b)))
  (cond
    [(null? (cdr L)) i]
    [else (+ i (CRT-helper (cdr L) m))])
)
...
(define (CRT L)
  (cond
    [(CRT-exists? L) (modulo (CRT-helper L (find-m L)) (find-m L))]
    [else -1])
)
