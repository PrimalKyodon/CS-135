...
(define (generate p q e)
  (list (list (* p q) e) (list (* p q) (mod-inverse e (lcm (- p 1) (- q 1)))))
)
...
(define (encrypt plaintext key)
  (map (lambda (ascii) (mod-exp (char->integer ascii) (cadr key) (car key))) (string->list plaintext))
)
...
(define (decrypt ciphertext key)
  (list->string (map (lambda (cipher) (integer->char (mod-exp cipher (cadr key) (car key)))) ciphertext))
)
