...
(display (eq? (wj-solvable? 9 12 6)  #t))
...
(define (waterjug cA cB g)
  (define (help fA fB)
    (cond
      [(= fB g) (list (list fA fB))]
      [(= fB cB) (cons (list fA fB) (help fA 0))]
      [(= 0 fA) (cons (list fA fB) (help cA fB))]
      [(> (+ fA fB) cB) (cons (list fA fB) (help (- (+ fA fB) cB) cB))]
      [else (cons (list fA fB) (help 0 (+ fA fB)))])
    )
  (help 0 0)
)
...
(define (factorize n)
  ...
  (cond
    [(or (= n 1) (= n 0) (= (factor 2) n)) (list n)]
    [else (cons (factor 2) (factorize (/ n (factor 2))))])
)
