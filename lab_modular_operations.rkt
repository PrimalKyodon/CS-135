...
;; 3 PTS
(define (cong-mod? m)
  (lambda (x y) (eq? (modulo x m) (modulo y m)))
)
...
(define (pulverizer a b)
  (cond
    [(= (modulo a b) 0) '(0 1)]
    [else (list (cadr (pulverizer b (modulo a b))) (+ (* (cadr (pulverizer b (modulo a b))) (- (quotient a b))) (car (pulverizer b (modulo a b)))))])
)
...
(define (mod-inverse a m)
  (define pulv (pulverizer a m))
  (cond
    [(= 1 (gcd a m)) (modulo (car pulv) m)]
    [else (error-inverse)])
)
